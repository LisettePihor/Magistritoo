...
def optimeeri_mets(X,y,n_iter=1000):
    parameetrid = {'n_estimators': [100, 300, 500, 600],
                   'max_depth': [None, 10, 20, 30, 50], 
                   'max_features': ['sqrt', 'log2', 0.5, 0.8],
                   'min_samples_split': [2, 5, 10],
                   'min_samples_leaf': [1, 2, 4]}
    grid = list(ParameterGrid(parameetrid))
    random.shuffle(grid)
    ...
    selected_params = grid[:n_iter]

    for g in tqdm(selected_params, desc="Optimeerin metsa", unit="mudel"):
    ...
    if os.path.exists(fail):
        ...
        while True:
            ...
            if len(uued_tunnused) <= 10:
                if mse > mse_parim or oob > oob_parim:
...
def otsustusmets(X_treening_idga, y_treening_idga, X_test_idga, y_test_idga, kombo_nr, jaotus, tunnused=None):
    ...
    if os.path.exists(fail):
        ...
        mudel = RandomForestRegressor(
            n_estimators=500, random_state=42, oob_score=True,
